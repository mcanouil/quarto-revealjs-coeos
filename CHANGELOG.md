...
- docs: Add a documentation website under `docs/`, built on the `atelier` project type and published to <https://m.canouil.dev/revealjs-coeos/>, including a deck built by the site itself.
...
- fix: Anchor the `index.qmd` and `_quarto.yml` rules in `.gitignore` to the repository root. Unanchored, they also matched `docs/index.qmd` and `docs/_quarto.yml`, which the documentation website needs.
